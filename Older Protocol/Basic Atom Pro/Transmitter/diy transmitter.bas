...
; New Data format stuff...
XBEEDATAVERSION			con 1       	; Data version...
MAXPACKETBYTEDIFF		con 4			; Max byte delta before we send full message...
...
UserSelectXPDeadZone:
	gosub XBeeTransNotReady		; tell client we are not ready
...
#endif
			goto _CTDP_MaybeSendNewDataAvail
		EndIf
#ifdef DEBUG
'		serout s_out, i9600, ["Recv:", hex CmdPacket(0)\2, hex CmdPacket(1)\2,hex CmdPacket(2)\2,hex CmdPacket(3)\2, 13]
		...
		if (bPacketType = XBEE_RECV_REQ_DATA )  then
			; First lets generate the checksum
			; Output the packet data
			; Packet(0) = XBEE_TRANS_DATA
			; Packet(1) = Checksum
			; Packet(2) = Sequence Number in here from request
			; Packet(3) = XBEEPACKETSIZE	; XBEEPACKETSIZE extra bytes
			; extra data...
			...
			fSendNewPacketMsg = FALSE												; Clear out that we have new data.
			fNewPacketMsgSent = FALSE												; We have not sent a new Msg since the last data

		elseif (bPacketType = XBEE_RECV_REQ_DATA2 )
		    ; Client supports new message formats.
            wMBit = 1;
            cChanged = 0;
            wMask = 0;    
			if fFirstDataPacket then
				fFirstDataPacket = 0
				cChanged = MAXPACKETBYTEDIFF + 1	; make sure we will send full packet...
			endif	
            for i=0 to XBEEPACKETSIZE-1 
                if (bPacket(i) <> bPacketLastSent(i)) then 
                     bPacketLastSent(i) = bPacket(i);
                     wMask  = wMask | wMBit; 
                     if (cChanged < MAXPACKETBYTEDIFF) then
                         bTemp(cChanged+2) = bPacket(i);
                     endif    
                     cChanged = cChanged + 1;
                endif
                wMBit = wMBit << 1;    // set up the next bit...
            next
            ; Now see what type of packet to send...
            if (cChanged=0) then
				gosub SendXbeePacket[XBEE_TRANS_NOTHIN_CHANGED, 0, 0]		; Ok we dumped the data to the the output
            elseif (cChanged <= MAXPACKETBYTEDIFF)
            	bTemp(0) = wMask.highbyte
            	bTemp(1) = wMask.lowbyte
				gosub SendXbeePacket[XBEE_TRANS_CHANGED_DATA, cChanged+2, @bTemp]		; Ok we dumped the data to the the output
            else
				gosub SendXbeePacket[XBEE_TRANS_DATA, XBEEPACKETSIZE, @bPacket]		; Ok we dumped the data to the the output
			endif
			gosub GetCurrentTime[], lTimerStart
		...
		elseif (bPacketType = XBEE_RECV_REQ_NEW ) 
			fNewPacketMsgMode = 1
		elseif (bPacketType = XBEE_RECV_REQ_NEW_OFF ) 
			fNewPacketMsgMode = 0
	#ifdef DEBUG
			serout s_out, i9600, ["NO Mode: ", hex fNewPacketMsgMode, 13]
	#endif
	
		; Packet to display a value/string on LCD of the remote...
		; BUGBUG:::::::::::: split into two commands. One for a value and other for string...
		elseif (bPacketType = XBEE_RECV_DISP_VAL ) 
		...
		elseif (bPacketType = XBEE_PLAY_SOUND )
		    ...
		    sound p9, [50\3000,50\3500, 30\4000]; XP: NO SPEAKER
			gosub ClearInputBuffer	; try to clear everything else out.
		endif
	endif

_CTDP_MaybeSendNewDataAvail:
	; Do we have new data to tell the client about?
	if fNewPacketMsgMode and fSendNewPacketMsg and (fNewPacketMsgSent = 0) then
#ifdef DEBUG
		serout s_out, i9600, ["New: ", hex fPacketChanged, 13]
#endif		
	
		; We are in new msg mode and the data has changed and we have not told the client about it yet
		gosub SendXbeePacket[XBEE_TRANS_NEW,  0, 0]		; no extra data no seq number so chksum = 0 also
		fNewPacketMsgSent = TRUE
	endif

	
	; we did not get anything or we did not get a complet packet...
	gosub GetCurrentTime[], lTimerEnd
	if (lTimerEnd - lTimerStart) > ((2000 * WTIMERTICSPERMSDIV) / WTIMERTICSPERMSMUL) then
'		gosub ClearInputBuffer			; clear out our buffer.
		
		gosub XBeeTransReady			; send out another ready signal.
	endif
	return
...
XBeeTransReady:
	...
	gosub SendXbeePacket[XBEE_TRANS_READY, 2, @XBeeMY]		; Real simple message 
	
	; Also transmit a Message to let the user know that we handle delta messages and beyond
	bTemp(0) = XBEEDATAVERSION
	gosub SendXbeePacket[XBEE_TRANS_DATA_VERSION, 1, @bTemp]		; Real simple message 
	fFirstDataPacket = 1							; flag to know that we have not sent full packet yet...
...
XBeeTransNotReady:
	...
	gosub SendXbeePacket[XBEE_TRANS_NOTREADY, 0, 0]		; Real simple message 
	return
