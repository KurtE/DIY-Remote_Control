...
void OLEDDisplayString(byte x, byte y, byte bFont, word wColor, const char *pb) {
  ...
  OLEDSerial.write(x);
  OLEDSerial.write(y);
  OLEDSerial.write(bFont);
  OLEDSerial.write((wColor >> 8) & 0xff);
  OLEDSerial.write(wColor & 0xff);
  OLEDSerial.write(pb);
  OLEDSerial.write((uint8_t)0x0);
  OLEDSetCommandActive();
}
...
void RemoteDisplay::Init(void) {                // lets initialize our display
  ...
  do {
    ...
    OLEDSerial.write((uint8_t)0x55);
    OLEDSerial.write((uint8_t)0x0);
  } 
  while (!OLEDWaitForAck(100));

  // Try the same init as forum member...
  OLEDSerial.write(0x45);
  OLEDSerial.write(0x00);       // Clear the screen
  OLEDWaitForAck(200);
  delay(200);

  // clear out anything that is hung up in the queue...
  while (OLEDSerial.read() != -1)
    ;
  //    OLEDSerial.flush();                             // get rid of any other data received
  OLEDSerial.write(0x56);
  OLEDSerial.write(0x01);
  OLEDSerial.write(0);       // Show version
...
void RemoteDisplay::SwitchToDisplayMode(byte bMode) {
  ...
  default:
    ...
    OLEDWaitForPreviousCommandToComplete(100);
    OLEDSerial.write(0x4f);
    OLEDSerial.write(1);             // set text as opaque
    OLEDSetCommandActive();
...
void RemoteDisplay::DisplayStatus(char *pbTitle, char *pb) {
  ...
  if (pbTitle) {
    ...
    DBGSerial.println("Scrool Screen");
    // Scroll up the stuff from the next line... use screen copy paste command.
    OLEDWaitForPreviousCommandToComplete(100);
    OLEDSerial.print("c");
    OLEDSerial.write(0);
    OLEDSerial.write(9*12);
    OLEDSerial.write(0);
    OLEDSerial.write(8*12);
    OLEDSerial.write(128);
    OLEDSerial.write(12);
    OLEDSetCommandActive();
  }
...
void RemoteDisplay::DisplayRemoteValue(byte iField,  word wVal) {
  ...
  cb = strlen(ab);
  if (cb < 5) 
    OLEDDrawRect(iField*8, 7*12, (iField + 5 - cb)*8-1, 10*12-1, DCLR_BLACK);    // should clear the rest of the line out.
  OLEDDisplayString(iField+5-cb, 7, 2, DCLR_WHITE, ab);         // draw the text

  DBGSerial.print("DisplayRemoteValue ");
  DBGSerial.println(wVal);
}
