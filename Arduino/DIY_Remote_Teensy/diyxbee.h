...
// Simplified version!
// Will output a datamessage every N milliseconds, no need for many of the old messages
// like ready or not ready.  We can detect that messages are received and will rely on that
//
//=============================================================================
#ifndef _DIYXBEE_H_
#define _DIYXBEE_H_
...
#define CXBEEDATAPACKETDELTAMS    26

//#define XBEE_TRANS_READY			0x01	// Transmitter is ready for requests.
// Optional uint16_t to use in ATDL command
...
typedef struct _diystate
{
  ...
  uint16_t        wDataPacketDeltaMS;    // How long should we wait between data packets   

  // Other information, could make static to file...
  ulong	          ulLastPacket;

  uint16_t        wXBeeMY;                              // Our My value
  uint16_t        wXBeeDL;                              // Current DL
